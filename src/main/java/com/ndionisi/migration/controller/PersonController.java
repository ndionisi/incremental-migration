...
public class PersonController {
    ...
    public PersonJpa createPersonJpa(@Valid @RequestBody PersonDto personDto) {
        PersonJpa person = new PersonJpa(personDto.getLastName(), personDto.getPhoneNumber());
        return personJpaRepository.save(person);
    }

    @PostMapping("/jdbc")
    public PersonJdbc createPersonJdbc(@Valid @RequestBody PersonDto personDto) {
        PersonJdbc person = new PersonJdbc(personDto.getLastName(), personDto.getPhoneNumber());
